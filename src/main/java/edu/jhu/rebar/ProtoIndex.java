...
/** A wrapper around a "root" protobuf object that maintains a
 * UUID-based index of all subobjects that define a 'uuid' field.
 *
 * In addition, the methods 'addField()' and 'setField()' may be used
 * to make monotonic changes to the protobuf object (or any contained
 * object with a uuid).  When such changes are made, the wrapped
 * object is replaced (since protobuf objects are immutable), and all
 * indices are updated appropraitely.  In addition, all changes are
 * tracked, allowing us to write a specific set of changes as the
 * stage output for a corpus or knowledge graph.
 *
 * A single ProtoIndex may be shared by multiple IndexedXYZ objects.
 * For example, an IndexedCommunication might use a ProtoIndex to
 * index the communication, and then use the same index when building
 * IndexedTokenization objects.  This ensures that all changes get
 * appropriately propagated all the way up to the "root" object.
 */
public final class ProtoIndex {
	...
	/** Pointers from messages to their parents.  We only bother to
	 * add messages to this if they have a UUID, or if any of their
	 * descendents have a UUID.  The primary purpose of this parent
	 * index is to allow us to make modifications.  
	 *
	 * (Note: we could make the value of this map contain more
	 * information -- e.g., the field that holds the message and the
	 * index for repeated fields.  This might make modifications
	 * faster, but would make indexing slower and memory usage
	 * larger.) */
	private final IdentityHashMap<Message, Message> parentIndex;

	/** Pointers to callback functions that should be called whenever
	 * we replace a given protobuf message with a new message.  This
	 * callback is called before we do the actual replacement. */
	 ...
	 * yet been saved to the Corpus.  This variable maps from UUIDs of
	...
	/** Interface used to handle a notification that is generated
	 * whenever the protobuf object with a specific UUID gets replaced
	 * with a new protobuf object.  (The UUID is specified when
	 * registering the ReplaceProtoCallback.) */
	...
	public void addField(Message target, FieldDescriptor field, Message fieldValue) 
		throws RebarException
	{
	 ...
	 * message must be contained in this indexed proto, and must have
	 * a UUID. */
	public void setField(Message target, FieldDescriptor field, Message fieldValue) 
		throws RebarException
	{
		// Input validity check:
		if (!parentIndex.containsKey(target))
	...
	/** Get the set of unsaved modifications, or null if no
	 * modifications have been made.  This should only be called when
	 * actually saving the modifications. */
	public Map<ModificationTarget, byte[]> getUnsavedModifications() {
		return unsavedModifications;
	}

	/** Clear the list of unsaved modifications.  This should be
	 * called once the modifications returned by
	 * getUnsavedModifications() have been saved. */
	...
	/** Return the protobuf object within this indexed proto that has
	...
	/** Return the IndexedProto for the given UUID if it has already
	 * been created; or null otherwise. */
	@SuppressWarnings("unchecked")
	public <T extends IndexedProto<?>> T getIndexedProto(Message key) throws RebarException {
		SoftReference<IndexedProto<?>> ref = indexedProtos.get(key);
		return (ref==null) ? null : (T)ref.get();
	}

	protected <T extends IndexedProto<?>> void registerIndexedProto(Message key, T indexedProto) throws RebarException {
	...
	private boolean buildIndex(Message msg) throws RebarException
	{
		...
		boolean addToParentIndex = ((msg instanceof Concrete.Edge) || 
									(msg instanceof Concrete.UndirectedAttributes) ||
									(msg instanceof Concrete.DirectedAttributes));
	...
	private void replaceAndUpdate(Message oldMsg, Message newMsg,
								 FieldDescriptor field, Message fieldValue) 
		throws RebarException 
	{
		...
		if (!parentIndex.containsKey(oldMsg))
			throw new RebarException("oldMsg is not in the parent index -- invalid target for modification");
	...
	/** Helper method for addField() and setField().  Walks up the
	 * chain of parents, replacing protobuf messages as necessary.
	 * Also updates any affected indices. */
		...
		throws RebarException
	{
		...
		if (oldMsg == root) {
			...
			throw new RebarException("oldMsg not found in parentIndex.get(oldMsg)!");
		}
	}

	/** Helper for replaceMessage() -- Given a message, modify all of
	 * its direct children to point to that message in the parent map.
	 * This is necessary whenever we replace a protobuf object with a
	 * new one, since the old parent pointers all point to the old
	 * one. */
	private void updateParentIndex(Message parent) 
		throws RebarException
	{
		for(Map.Entry<FieldDescriptor,Object> entry: parent.getAllFields().entrySet()) {
			FieldDescriptor field = entry.getKey();
	...
	private final static ModificationTarget ZERO_UUID_MODIFICATION_TARGET = 
		new ModificationTarget(Concrete.UUID.newBuilder().setHigh(0).setLow(0).build());

	private void recordModification(Message target, FieldDescriptor field, Message fieldValue) 
		throws RebarException
	{
		//if ((target instanceof Concrete.UUID) || (target instanceof Concrete.EdgeId))
		...
		if (uuid != null) {
			...
			ModificationTarget mTarget = new ModificationTarget(((Concrete.Edge)target).getEdgeId());
			recordModification(mTarget, field, fieldValue);
		} else {
			...
			throw new RebarException("Invalid merge target (not reachable via non-"+
	...
	private void recordModification(ModificationTarget target, FieldDescriptor field, Message fieldValue) 
		throws RebarException
	{
		...
		throws RebarException 
	{
		...
		if (uuid == null)
			...
			*/
			return new RebarException(protoDebugString(m)+" not found inside "+
								  protoDebugString(root));
		}
	}

	private RebarException reportDuplicateField(Message target, FieldDescriptor field) 
		throws RebarException 
	{
