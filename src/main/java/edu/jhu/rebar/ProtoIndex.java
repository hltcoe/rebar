...
/**
 * A wrapper around a "root" protobuf object that maintains a UUID-based index
 * of all subobjects that define a 'uuid' field.
 * 
 * In addition, the methods 'addField()' and 'setField()' may be used to make
 * monotonic changes to the protobuf object (or any contained object with a
 * uuid). When such changes are made, the wrapped object is replaced (since
 * protobuf objects are immutable), and all indices are updated appropraitely.
 * In addition, all changes are tracked, allowing us to write a specific set of
 * changes as the stage output for a corpus or knowledge graph.
 * 
 * A single ProtoIndex may be shared by multiple IndexedXYZ objects. For
 * example, an IndexedCommunication might use a ProtoIndex to index the
 * communication, and then use the same index when building IndexedTokenization
 * objects. This ensures that all changes get appropriately propagated all the
...
public final class ProtoIndex {
    ...
    /**
     * Pointers from messages to their parents. We only bother to add messages
     * to this if they have a UUID, or if any of their descendents have a UUID.
     * The primary purpose of this parent index is to allow us to make
     * modifications.
     * 
     * (Note: we could make the value of this map contain more information --
     * e.g., the field that holds the message and the index for repeated fields.
     * This might make modifications faster, but would make indexing slower and
     * memory usage larger.)
     */
    private final IdentityHashMap<Message, Message> parentIndex;

    /**
     * Pointers to callback functions that should be called whenever we replace
     * a given protobuf message with a new message. This callback is called
     * before we do the actual replacement.
     */
    private final IdentityHashMap<Message, ReplaceProtoCallback> replaceProtoCallbacks;

    /**
     * A list of modifications that have been made but that have not yet been
     * saved to the Corpus. This variable maps from UUIDs of modified objects to
     * serialized protobuf messages.
     */
    private Map<ModificationTarget, byte[]> unsavedModifications = null;
    ...
    public ProtoIndex(Message root) throws RebarException {
        this.root = root;
        ...
        /*
         * for (Map.Entry<Message,Message> p: parentIndex.entrySet()) {
         * System.err.println(protoDebugString(p.getKey())+"'s parent is "+
    ...
    /**
     * Interface used to handle a notification that is generated whenever the
     * protobuf object with a specific UUID gets replaced with a new protobuf
     * object. (The UUID is specified when registering the
     * ReplaceProtoCallback.)
     */
    interface ReplaceProtoCallback {
        public void replace(Message oldMsg, Message newMsg)
                throws RebarException;
    }

    public void registerCallback(Message message, ReplaceProtoCallback callback)
            throws RebarException {
    ...
    /**
     * Append a new value to a specified repeated field in given target message.
     * The target message must be contained in this indexed proto, and must have
     * a UUID.
     */
    public void addField(Message target, FieldDescriptor field,
            Message fieldValue) throws RebarException {
        // Input validity check:
        if (!parentIndex.containsKey(target))
            throw reportBadFieldTarget(target, "addField");
        // Make the change.
        Message newTarget = target.toBuilder()
                .addRepeatedField(field, fieldValue).build();
        replaceAndUpdate(target, newTarget, field, fieldValue);
        // Final sanity check
        Object newFieldValue = newTarget.getRepeatedField(field,
                newTarget.getRepeatedFieldCount(field) - 1);
        if (newFieldValue != fieldValue)
            throw new RebarException("Identity changed in unexpected way");
    }

    /**
     * Set the value of a specified optional field in given target message. The
     * field must not already have a value. The target message must be contained
     * in this indexed proto, and must have a UUID.
     */
    public void setField(Message target, FieldDescriptor field,
            Message fieldValue) throws RebarException {
        // Input validity check:
        if (!parentIndex.containsKey(target))
        ...
        // Make the change.
        Message newTarget = target.toBuilder().setField(field, fieldValue)
                .build();
        replaceAndUpdate(target, newTarget, field, fieldValue);
        // Final sanity check
    ...
    /**
     * Get the set of unsaved modifications, or null if no modifications have
     * been made. This should only be called when actually saving the
     * modifications.
     */
    ...
    /**
     * Clear the list of unsaved modifications. This should be called once the
     * modifications returned by getUnsavedModifications() have been saved.
     */
    public void clearUnsavedModifications() {
        unsavedModifications = null;
    }
    ...
    /**
     * Return the protobuf object within this indexed proto that has the
     * specified UUID, or null if no such object is found.
     */
    ...
    /**
     * Return the IndexedProto for the given UUID if it has already been
     * created; or null otherwise.
     */
    @SuppressWarnings("unchecked")
    public <T extends IndexedProto<?>> T getIndexedProto(Message key)
            throws RebarException {
        SoftReference<IndexedProto<?>> ref = indexedProtos.get(key);
        return (ref == null) ? null : (T) ref.get();
    }

    protected <T extends IndexedProto<?>> void registerIndexedProto(
            Message key, T indexedProto) throws RebarException {
        indexedProtos
                .put(key, new SoftReference<IndexedProto<?>>(indexedProto));
    }
    ...
    private boolean buildIndex(Message msg) throws RebarException {
        ...
        for (Map.Entry<FieldDescriptor, Object> entry : msg.getAllFields()
            ...
            if (field.getName().equals("uuid")) {
                ...
                if (uuidIndex.put(uuid, msg) != null)
                    throw new RebarException("Duplicate UUID detected: " + uuid);
    ...
    private void replaceAndUpdate(Message oldMsg, Message newMsg,
            FieldDescriptor field, Message fieldValue) throws RebarException {
        ...
        if (!parentIndex.containsKey(oldMsg))
            throw new RebarException(
                    "oldMsg is not in the parent index -- invalid target for modification");
    ...
    /**
     * Helper method for addField() and setField(). Walks up the chain of
     * parents, replacing protobuf messages as necessary. Also updates any
     * affected indices.
     */
    private void replaceMessage(Message oldMsg, Message newMsg)
            throws RebarException {
        ...
        if (oldMsg == root) {
            ...
            assert (parent != null);
            for (Map.Entry<FieldDescriptor, Object> entry : parent
                    .getAllFields().entrySet()) {
                FieldDescriptor field = entry.getKey();
                if (field.getType() == FieldDescriptor.Type.MESSAGE) {
                    if (field.isRepeated()) {
                        @SuppressWarnings("unchecked")
                        List<Message> children = (List<Message>) entry
                                .getValue();
                        for (int i = 0; i < children.size(); i++) {
                            Message child = children.get(i);
                            if (child == oldMsg) {
                                Message newParent = parent.toBuilder()
                                        .setRepeatedField(field, i, newMsg)
                                        .build();
            ...
            throw new RebarException(
                    "oldMsg not found in parentIndex.get(oldMsg)!");
        }
    }

    /**
     * Helper for replaceMessage() -- Given a message, modify all of its direct
     * children to point to that message in the parent map. This is necessary
     * whenever we replace a protobuf object with a new one, since the old
     * parent pointers all point to the old one.
     */
    private void updateParentIndex(Message parent) throws RebarException {
        for (Map.Entry<FieldDescriptor, Object> entry : parent.getAllFields()
                .entrySet()) {
            FieldDescriptor field = entry.getKey();
            if (field.getType() == FieldDescriptor.Type.MESSAGE) {
                if (field.isRepeated()) {
    ...
    private final static ModificationTarget ZERO_UUID_MODIFICATION_TARGET = new ModificationTarget(
            Concrete.UUID.newBuilder().setHigh(0).setLow(0).build());

    private void recordModification(Message target, FieldDescriptor field,
            Message fieldValue) throws RebarException {
        // if ((target instanceof Concrete.UUID) || (target instanceof
        // Concrete.EdgeId))
        // throw new RebarException("You should not be editing UUIDS!");
        ...
        if (uuid != null) {
            ...
            if (field.isRepeated())
                newValue = target.newBuilderForType()
                        .addRepeatedField(field, fieldValue).buildPartial();
            else
                newValue = target.newBuilderForType()
                        .setField(field, fieldValue).buildPartial();
            ...
            throw new RebarException(
                    "Invalid merge target (not reachable via non-"
                            + "repeated fields from a message with a UUID");
        }
    }
    ...
    private void recordModification(ModificationTarget target,
            FieldDescriptor field, Message fieldValue) throws RebarException {
        if (unsavedModifications == null)
            unsavedModifications = new HashMap<ModificationTarget, byte[]>();
        final int fieldNum = field.getNumber();
        final int size = CodedOutputStream.computeMessageSize(fieldNum,
                fieldValue);
    ...
    private String protoDebugString(Message m) {
        ...
        if (uuid != null)
            return protoMessageType + "(uuid=" + IdUtil.uuidToString(uuid)
                    + ")@" + identityHash;
        else
    ...
    private RebarException reportBadFieldTarget(Message m, String methodName)
        ...
        Concrete.UUID uuid = IdUtil.getUUIDOrNull(m);
        if (uuid == null)
            return new RebarException(protoDebugString(m)
                    + " is not a valid target " + "for " + methodName
                    + " because it does not " + "have a UUID.");
        else {
            /*
             * System.err.println(root); for (Map.Entry<Message,Message> p:
             * parentIndex.entrySet()) {
             * System.err.println(protoDebugString(p.getKey())+"'s parent is "+
             * protoDebugString(p.getValue())); }
             */
            return new RebarException(protoDebugString(m)
                    + " not found inside " + protoDebugString(root));
        }
    }

    private RebarException reportDuplicateField(Message target,
            FieldDescriptor field) throws RebarException {
        throw new RebarException(protoDebugString(target) + " already has a "
                + field.getName() + " value");
    }
}
