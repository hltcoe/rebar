...
/** Remote Procedure Call (RPC) server that uses unnamed pipes (aka
 * stdin and stdout) to communicate.  This class is meant to be
 * spawned as a subprocess.  See rpc.proto for details about the
...
class UnnamedPipeRPC {
	...
	private static final RPCResponse okResponse = 
		RPCResponse.newBuilder().build();
	...
	private void close() throws RebarException {
		...
		for (Corpus.Initializer x: corpusInitializers.values()) x.close();
		for (Corpus.Reader x: corpusReaders.values()) x.close();
		for (Corpus.DiffWriter x: corpusDiffWriters.values()) x.close();
		for (Graph.Initializer x: graphInitializers.values()) x.close();
		for (Graph.Reader x: graphReaders.values()) x.close();
		for (Graph.DiffWriter x: graphDiffWriters.values()) x.close();
		for (Corpus c: corpora.values()) c.close();
		for (Graph g: graphs.values()) g.close();
	...
	private RPCResponse handleRequest(RPCRequest request) throws RebarException {
		...
		if (request.hasCommunicationIteratorNext()) return handleCommunicationIteratorNext(request.getCommunicationIteratorNext());
		else if (request.hasVertexIteratorNext()) return handleVertexIteratorNext(request.getVertexIteratorNext());
		else if (request.hasEdgeIteratorNext()) return handleEdgeIteratorNext(request.getEdgeIteratorNext());

		// 2. Iterator has-next methods
		else if (request.hasCommunicationIteratorHasNext()) return handleCommunicationIteratorHasNext(request.getCommunicationIteratorHasNext());
		else if (request.hasVertexIteratorHasNext()) return handleVertexIteratorHasNext(request.getVertexIteratorHasNext());
		else if (request.hasEdgeIteratorHasNext()) return handleEdgeIteratorHasNext(request.getEdgeIteratorHasNext());

		// 3. Write methods
		else if (request.hasCorpusDiffWriterWriteCommunicationDiff()) return handleCorpusDiffWriterWriteCommunicationDiff(request.getCorpusDiffWriterWriteCommunicationDiff());
		else if (request.hasGraphDiffWriterWriteVertexDiff()) return handleGraphDiffWriterWriteVertexDiff(request.getGraphDiffWriterWriteVertexDiff());
		else if (request.hasGraphDiffWriterWriteEdgeDiff()) return handleGraphDiffWriterWriteEdgeDiff(request.getGraphDiffWriterWriteEdgeDiff());
		
		// 4. Everything else
		else if (request.hasGetCorpus()) return handleGetCorpus(request.getGetCorpus());
		else if (request.hasMakeCorpus()) return handleMakeCorpus(request.getMakeCorpus());
		else if (request.hasDeleteCorpus()) return handleDeleteCorpus(request.getDeleteCorpus());
		else if (request.hasListCorpora()) return handleListCorpora(request.getListCorpora());
		// Corpus methods
		else if (request.hasCorpusClose()) return handleCorpusClose(request.getCorpusClose());
		else if (request.hasCorpusGetSize()) return handleCorpusGetSize(request.getCorpusGetSize());
		else if (request.hasCorpusGetStages()) return handleCorpusGetStages(request.getCorpusGetStages());
		else if (request.hasCorpusMakeStage()) return handleCorpusMakeStage(request.getCorpusMakeStage());
		else if (request.hasCorpusDeleteStage()) return handleCorpusDeleteStage(request.getCorpusDeleteStage());
		else if (request.hasCorpusMarkStagePublic()) return handleCorpusMarkStagePublic(request.getCorpusMarkStagePublic());
		else if (request.hasCorpusMakeInitializer()) return handleCorpusMakeInitializer(request.getCorpusMakeInitializer());
		else if (request.hasCorpusMakeReader()) return handleCorpusMakeReader(request.getCorpusMakeReader());
		else if (request.hasCorpusMakeDiffWriter()) return handleCorpusMakeDiffWriter(request.getCorpusMakeDiffWriter());
		else if (request.hasCorpusReadComIdSet()) return handleCorpusReadComIdSet(request.getCorpusReadComIdSet());
		else if (request.hasCorpusLookupComIdSet()) return handleCorpusLookupComIdSet(request.getCorpusLookupComIdSet());
		else if (request.hasCorpusRegisterComIdSet()) return handleCorpusRegisterComIdSet(request.getCorpusRegisterComIdSet());
		else if (request.hasCorpusGetNumCommunications()) return handleCorpusGetNumCommunications(request.getCorpusGetNumCommunications());
		else if (request.hasCorpusGetComIdSetNames()) return handleCorpusGetComIdSetNames(request.getCorpusGetComIdSetNames());
		// Corpus.Initializer Methods
		else if (request.hasCorpusInitializerClose()) return handleCorpusInitializerClose(request.getCorpusInitializerClose());
		else if (request.hasCorpusInitializerAddCommunication()) return handleCorpusInitializerAddCommunication(request.getCorpusInitializerAddCommunication());
		// Corpus.Reader methods
		else if (request.hasCorpusReaderClose()) return handleCorpusReaderClose(request.getCorpusReaderClose());
		else if (request.hasCorpusReaderLoadCommunication()) return handleCorpusReaderLoadCommunication(request.getCorpusReaderLoadCommunication());
		else if (request.hasCorpusReaderLoadCommunications()) return handleCorpusReaderLoadCommunications(request.getCorpusReaderLoadCommunications());
		// Corpus.DiffWriter methods
		else if (request.hasCorpusDiffWriterClose()) return handleCorpusDiffWriterClose(request.getCorpusDiffWriterClose());
		else if (request.hasCorpusDiffWriterFlush()) return handleCorpusDiffWriterFlush(request.getCorpusDiffWriterFlush());
		// Graph Methods (Static)
		else if (request.hasMakeGraph()) return handleMakeGraph(request.getMakeGraph());
		else if (request.hasGetGraph()) return handleGetGraph(request.getGetGraph());
		else if (request.hasDeleteGraph()) return handleDeleteGraph(request.getDeleteGraph());
		else if (request.hasListGraphs()) return handleListGraphs(request.getListGraphs());
		// Graph Methods
		else if (request.hasGraphClose()) return handleGraphClose(request.getGraphClose());
		else if (request.hasGraphGetStages()) return handleGraphGetStages(request.getGraphGetStages());
		else if (request.hasGraphMakeStage()) return handleGraphMakeStage(request.getGraphMakeStage());
		else if (request.hasGraphDeleteStage()) return handleGraphDeleteStage(request.getGraphDeleteStage());
		else if (request.hasGraphMarkStagePublic()) return handleGraphMarkStagePublic(request.getGraphMarkStagePublic());
		else if (request.hasGraphMakeInitializer()) return handleGraphMakeInitializer(request.getGraphMakeInitializer());
		else if (request.hasGraphMakeReader()) return handleGraphMakeReader(request.getGraphMakeReader());
		else if (request.hasGraphMakeDiffWriter()) return handleGraphMakeDiffWriter(request.getGraphMakeDiffWriter());
		else if (request.hasGraphGetEdgesWithOneVertexIn()) return handleGraphGetEdgesWithOneVertexIn(request.getGraphGetEdgesWithOneVertexIn());
		else if (request.hasGraphGetEdgesWithBothVerticesIn()) return handleGraphGetEdgesWithBothVerticesIn(request.getGraphGetEdgesWithBothVerticesIn());
		else if (request.hasReadVertexIdSet()) return handleReadVertexIdSet(request.getReadVertexIdSet());
		else if (request.hasLookupVertexIdSet()) return handleLookupVertexIdSet(request.getLookupVertexIdSet());
		else if (request.hasRegisterVertexIdSet()) return handleRegisterVertexIdSet(request.getRegisterVertexIdSet());
		else if (request.hasGetVertexIdSetNames()) return handleGetVertexIdSetNames(request.getGetVertexIdSetNames());
		else if (request.hasReadEdgeIdSet()) return handleReadEdgeIdSet(request.getReadEdgeIdSet());
		else if (request.hasLookupEdgeIdSet()) return handleLookupEdgeIdSet(request.getLookupEdgeIdSet());
		else if (request.hasRegisterEdgeIdSet()) return handleRegisterEdgeIdSet(request.getRegisterEdgeIdSet());
		else if (request.hasGetEdgeIdSetNames()) return handleGetEdgeIdSetNames(request.getGetEdgeIdSetNames());
		else if (request.hasGraphGetNumVertices()) return handleGraphGetNumVertices(request.getGraphGetNumVertices());
		else if (request.hasGraphGetSize()) return handleGraphGetSize(request.getGraphGetSize());
		// Graph.Initializer Methods
		else if (request.hasGraphInitializerClose()) return handleGraphInitializerClose(request.getGraphInitializerClose());
		else if (request.hasGraphInitializerAddVertex()) return handleGraphInitializerAddVertex(request.getGraphInitializerAddVertex());
		// Graph.Reader Methods
		else if (request.hasGraphReaderClose()) return handleGraphReaderClose(request.getGraphReaderClose());
		else if (request.hasGraphReaderLoadVertex()) return handleGraphReaderLoadVertex(request.getGraphReaderLoadVertex());
		else if (request.hasGraphReaderLoadVertices()) return handleGraphReaderLoadVertices(request.getGraphReaderLoadVertices());
		else if (request.hasGraphReaderLoadEdge()) return handleGraphReaderLoadEdge(request.getGraphReaderLoadEdge());
		else if (request.hasGraphReaderLoadEdges()) return handleGraphReaderLoadEdges(request.getGraphReaderLoadEdges());
		// Graph.DiffWriter Methods
		else if (request.hasGraphDiffWriterClose()) return handleGraphDiffWriterClose(request.getGraphDiffWriterClose());
		else if (request.hasGraphDiffWriterFlush()) return handleGraphDiffWriterFlush(request.getGraphDiffWriterFlush());
		// IdSet Methods
		else if (request.hasComIdSetEnumerate()) return handleComIdSetEnumerate(request.getComIdSetEnumerate());
		else if (request.hasVertexIdSetEnumerate()) return handleVertexIdSetEnumerate(request.getVertexIdSetEnumerate());
		else if (request.hasEdgeIdSetEnumerate()) return handleEdgeIdSetEnumerate(request.getEdgeIdSetEnumerate());
		// Close
		else if (request.hasClose()) return okResponse;
		else throw new RebarException("Unexpected request type");
	}

	/** Assign an identifier to an object, and store it in an index
	...
	private Collection<String> parseComIdCollection(Corpus corpus, ConcreteRpc.ComIdCollection comList) 
	...
	private Collection<Concrete.UUID> parseVertexIdCollection(Graph graph, ConcreteRpc.VertexIdCollection vertexList)
		throws RebarException
	{
	...
	private Collection<Concrete.EdgeId> parseEdgeIdCollection(Graph graph, ConcreteRpc.EdgeIdCollection edgeList)
		throws RebarException
	{
	...
	private void addStageOwnership(StageOwnership stageOwnership, RPCResponse.Builder response) {
		...
		for (Map.Entry<StageOwnership.FieldValuePointer, Stage> e: stageOwnership.entrySet()) {
			...
			while (fvp != null) {
				segments.add(ConcreteRpc.StageOwnershipMap.Segment.newBuilder()
							 .setFieldTag(fvp.field.getNumber())
							 .setIndex(fvp.index).build());
				fvp = fvp.parent;
			}
			Collections.reverse(segments);
			ownershipMap.addEntryBuilder()
				.setStageId(e.getValue().getStageId())
				.addAllPathSegment(segments);
		}
	}
	...
	private RPCResponse handleCorpusMakeStage(ConcreteRpc.Corpus.MakeStage request) throws RebarException {
		...
		Stage newStage = self.makeStage(request.getStageName(), 
										request.getStageVersion(), 
										dependencies,
	...
	private RPCResponse handleCorpusMakeInitializer(ConcreteRpc.Corpus.MakeInitializer request) throws RebarException {
		...
		return RPCResponse.newBuilder()
			.setCorpusInitializer(buildRpcCorpusInitializer(request.getSelf(), id))
			.build();
	}
	...
	private RPCResponse handleCorpusMakeReader(ConcreteRpc.Corpus.MakeReader request) throws RebarException {
		...
		return RPCResponse.newBuilder()
			.setCorpusReader(buildRpcCorpusReader(request.getSelf(), id))
			.build();
	}
	...
	private RPCResponse handleCorpusMakeDiffWriter(ConcreteRpc.Corpus.MakeDiffWriter request) throws RebarException {
		...
		return RPCResponse.newBuilder()
			.setCorpusDiffWriter(buildRpcCorpusDiffWriter(request.getSelf(), id))
			.build();
	}
	...
	private RPCResponse handleCorpusReadComIdSet(ConcreteRpc.Corpus.ReadComIdSet request) throws RebarException {
		...
		return RPCResponse.newBuilder()
			.setComIdSet(buildRpcComIdSet(request.getSelf(), id))
			.build();
	}
	...
	private RPCResponse handleCorpusReaderLoadCommunications(ConcreteRpc.CorpusReader.LoadCommunications request) throws RebarException {
		...
		return RPCResponse.newBuilder()
			.setCommunicationIterator(buildRpcCommunicationIterator(request.getSelf(), id))
			.build();
	}
	...
	private RPCResponse handleCorpusDiffWriterClose(ConcreteRpc.CorpusDiffWriter.Close request) throws RebarException {
		...
		if (self==null) throw new RebarException("CorpusDiffWriter not found!");
		self.close();
		return okResponse;
	}
	...
	private RPCResponse handleGetGraph(ConcreteRpc.GetGraph request) throws RebarException {
		...
		return RPCResponse.newBuilder()
			.setGraph(buildRpcGraph(id)).build();
	}
	...
	private RPCResponse handleListGraphs(ConcreteRpc.ListGraphs request) throws RebarException {
		return RPCResponse.newBuilder()
			.addAllGraphName(Graph.Factory.list()).build();
	}
	...
	private RPCResponse handleGraphMakeInitializer(ConcreteRpc.Graph.MakeInitializer request) throws RebarException {
		...
		return RPCResponse.newBuilder()
			.setGraphInitializer(buildRpcGraphInitializer(request.getSelf(), id))
			.build();
	}
	...
	private RPCResponse handleGraphMakeReader(ConcreteRpc.Graph.MakeReader request) throws RebarException {
		...
		return RPCResponse.newBuilder()
			.setGraphReader(buildRpcGraphReader(request.getSelf(), id))
			.build();
	}
	...
	private RPCResponse handleGraphMakeDiffWriter(ConcreteRpc.Graph.MakeDiffWriter request) throws RebarException {
		...
		return RPCResponse.newBuilder()
			.setGraphDiffWriter(buildRpcGraphDiffWriter(request.getSelf(), id))
			.build();
	}
	...
	private RPCResponse handleReadVertexIdSet(ConcreteRpc.Graph.ReadVertexIdSet request) throws RebarException {
		...
		return RPCResponse.newBuilder()
			.setVertexIdSet(buildRpcVertexIdSet(request.getSelf(), id))
			.build();
	}
	...
	private RPCResponse handleReadEdgeIdSet(ConcreteRpc.Graph.ReadEdgeIdSet request) throws RebarException {
		...
		return RPCResponse.newBuilder()
			.setEdgeIdSet(buildRpcEdgeIdSet(request.getSelf(), id))
			.build();
	}
	...
	private RPCResponse handleGraphReaderLoadVertices(ConcreteRpc.GraphReader.LoadVertices request) throws RebarException {
		...
		return RPCResponse.newBuilder()
			.setVertexIterator(buildRpcVertexIterator(request.getSelf(), id))
			.build();
	}
	...
	private RPCResponse handleGraphReaderLoadEdges(ConcreteRpc.GraphReader.LoadEdges request) throws RebarException {
		...
		return RPCResponse.newBuilder()
			.setEdgeIterator(buildRpcEdgeIterator(request.getSelf(), id))
			.build();
	}
