...
public class IdUtil {
	...
	/** Return a new EdgeId for the (undirected) edge connecting the
	 * given pair of vertices.  The two vertex UUIDs will be sorted
	 * before the EdgeId is created, to ensure that v1 is the lesser
	 * UUID and v1 is the greater UUID (as is required by the EdgeId
	 * type).  If you wish to create an identifier that identifies an
	 * edge and an associated direction, then use buildDirectedEdgeId
	 * instead.  */
	...
	/** Return a new EdgeId for the undirected edge identified by the
	 * given directed edge id.  This essentially "strips off" the
	 * direction information, by storing the two edge vertex UUIDs in
	 * a canonical order. */
	public static Concrete.EdgeId buildEdgeId(Concrete.DirectedEdgeId edgeId) {
		return buildEdgeId(edgeId.getSrc(), edgeId.getDst());
	}

	/** Return true if the given edgeId is valid.  An edgeId is
	 * invalid if v2 is lesser than v1. */
	public static boolean edgeIdIsValid(Concrete.EdgeId edgeId) { 
		return uuidsAreOrdered(edgeId.getV1(), edgeId.getV2());
	}

	/** Return a new DirectedEdgeId for the given source and
	 * destination vertices.  Note: if you wish to create an
	 * identifier that identifies an edge *without* an associated
	 * direction, then use buildEdgeId instead. */
	...
	/** Return a new DirectedEdgeId for the given edge, with the
	 * specified direction.  If direction is V1_TO_V2, then edgeId.v1
	 * will be used as the source and edgeId.v2 will be used as the
	 * destination.  If direction is V2_TO_V1, then edgeId.v2 will be
	 * used as the source and edgeId.v1 will be used as the
	 * destination.*/
	public static Concrete.DirectedEdgeId buildDirectedEdgeId(Concrete.EdgeId edgeId, Concrete.DirectedEdgeId.Direction direction) {
		if (direction == Concrete.DirectedEdgeId.Direction.V1_TO_V2) {
			return Concrete.DirectedEdgeId.newBuilder()
				.setSrc(edgeId.getV1()).setDst(edgeId.getV2()).build();
		} else {
			return Concrete.DirectedEdgeId.newBuilder()
				.setSrc(edgeId.getV2()).setDst(edgeId.getV1()).build();
		}
	}

	public static Concrete.DirectedEdgeId.Direction getEdgeDirection(Concrete.DirectedEdgeId edgeId) {
		return ((uuidsAreOrdered(edgeId.getSrc(), edgeId.getDst())) ?
				Concrete.DirectedEdgeId.Direction.V1_TO_V2:
				Concrete.DirectedEdgeId.Direction.V2_TO_V1);
	}

	/** Return true if the UUID v1 is lesser than or equal to the UUID
	 * v2. */
	public static boolean uuidsAreOrdered(Concrete.UUID v1, Concrete.UUID v2) {
		...
		return (MathUtil.unsignedLessThan(v1.getHigh(), v2.getHigh()) || 
					 ((v1.getHigh()==v2.getHigh()) && 
					  !MathUtil.unsignedLessThan(v2.getLow(), v1.getLow())));
	}

	/** Return a java UUID extracted from the "uuid" field of the
	 * given protobuf object.  Throw RebarException if the given 
	 * protobuf object has no "uuid" field. */
	...
	/** Return a java UUID extracted from the "uuid" field of the
	 * given protobuf object.  Return null if the given 
	 * protobuf object has no "uuid" field. */
	...
	private static Map<Descriptor, FieldDescriptor> uuidFields 
		= new HashMap<Descriptor, FieldDescriptor>();
