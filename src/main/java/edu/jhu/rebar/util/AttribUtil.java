...
public class AttribUtil {
	...
	//private static final Descriptor Xx_ATTRIBUTE = Concrete.XxAttribute.getDescriptor();

	public static Message buildAttribute(FieldDescriptor field, Object value, String toolName) 
		throws RebarException
	{
		return buildAttribute(field, value, buildMetadata(toolName));
	}

	public static Message buildAttribute(FieldDescriptor field, Object value, String toolName, float confidence) 
		throws RebarException
	{
	...
	{
		if (field == null)
			throw new RebarException("FieldDescriptor is NULL!  This probably means that you had a typo "+
	...
	private static List<Concrete.StringFloatMapAttribute.Entry> buildStringFloatMap(Object object) 
		throws RebarException
	{
		try {
			...
			for (Map.Entry<Object,Object> entry: map.entrySet()) {
				result.add(Concrete.StringFloatMapAttribute.Entry.newBuilder()
						   .setKey((String)entry.getKey())
						   .setValue((Float)entry.getValue())
						   .build());
			}
			return result;
		} catch (ClassCastException e) {
			throw new RebarException("Expected a Map<String,Float>!");
		}
	}

	public static Concrete.AttributeMetadata buildMetadata(String toolName, float confidence)
	{
		return Concrete.AttributeMetadata.newBuilder()
			.setTool(toolName)
