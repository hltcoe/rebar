...
/**
 * A class that records what stage created each portion of a protobuf object.
 * This is intended primarily for display purposes, when users are looking at
 * the contents of a corpus or graph, and wish to see which pieces were
 * generated by which stages.
 * */
public class StageOwnership extends HashMap<StageOwnership.FieldValuePointer, Stage> {
    ...
    /**
     * A pointer to a single field value in a complex protobuf object, expressed
     * as a path from the root via intermediate fields to the value. Each
     * segment in this path consists of a field descriptor and an optional index
     * (used for repeated fields, and set to zero for non-repeated fields).
     * 
