...
public class TwitterTokenizer {
  ...
  */
  ...
  public static SimpleImmutableEntry<Pattern,String>[] getUnicodePatterns () throws IOException {
	...
	BufferedReader reader = FileUtil.getReaderFromInputStream(
			TwitterTokenizer.class
			.getClassLoader()
    ...
    char[] pair;
    String regexp;
    Vector<SimpleImmutableEntry<Pattern,String>> p = new Vector<SimpleImmutableEntry<Pattern,String>>();
  ...
  private static SimpleImmutableEntry<String,String>[] recursiveTokenize (String text,
    ...
    if (index < patterns.length) {
      ...
      String textFragment = "";
      if (groupCount > 0) {
        Vector<SimpleImmutableEntry<String,String>[]> arrays = 
        		new Vector<SimpleImmutableEntry<String,String>[]>();
        int lastEnd = 0;
        while (matcher.find()) {
