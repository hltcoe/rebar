...
public class TwitterTokenizer {
    ...
    public static List<PatternStringTuple> getUnicodePatterns() throws IOException {
        List<PatternStringTuple> tupleList = new ArrayList<>();
        BufferedReader reader = FileUtil.getReaderFromInputStream(TwitterTokenizer.class.getClassLoader()
    ...
    public static String[] tokenizeTweet(String text, TokenizationType tokenization) throws IOException {
        ...
        if (!RebarConfiguration.getTwitterTokenizerRw()) {
            for (int i = 0; i < x.size(); i++)
                y[i] = x.get(i).getKey();
        } else {
            for (int i = 0; i < x.size(); i++) {
                if (x.get(i).getValue() != null)
                    y[i] = "[" + x.get(i).getValue() + "]";
                else
                    y[i] = x.get(i).getKey();
            }
        }
    ...
    private static List<SimpleImmutableEntry<String, String>> recursiveTokenize(String text, int index, TokenizationType tokenization)
        ...
        if (index < tupleList.size()) {
            ...
            String textFragment = "";
            if (groupCount > 0) {
                List<List<SimpleImmutableEntry<String, String>>> arrays = new ArrayList<>();
                int lastEnd = 0;
                while (matcher.find()) {
                    ...
                    List<SimpleImmutableEntry<String, String>> tmpList = new ArrayList<>();
                    tmpList.add(new SimpleImmutableEntry<>(matcher.group(), tag));
                    arrays.add(tmpList);
                    lastEnd = matcher.end();
                }
            ...
            List<SimpleImmutableEntry<String, String>> y = new ArrayList<>(x.length);
            for (String token : x)
                y.add(new SimpleImmutableEntry<String, String>(token, null));
            return y;
        }
    }

    public static List<SimpleImmutableEntry<String, String>> concatAll(List<List<SimpleImmutableEntry<String, String>>> arrays) {
        int totalLength = 0;
        for (List<SimpleImmutableEntry<String, String>> array : arrays)
            totalLength += array.size();
        
        List<SimpleImmutableEntry<String, String>> result = new ArrayList<>(totalLength);
        for (List<SimpleImmutableEntry<String, String>> array : arrays)
            result.addAll(array);
