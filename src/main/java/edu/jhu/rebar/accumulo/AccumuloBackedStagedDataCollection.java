...
/**
 * Base class for the Accumulo backed Corpus and Graph classes.
 * This class is not indended to be directly instantiated.
 */
public class AccumuloBackedStagedDataCollection implements StagedDataCollection {
	...
	protected AccumuloBackedStagedDataCollection(String name, boolean create,
												 String tablePrefix, String[] tableSuffixes,
												 String [] protoTableSuffixes,
												 byte[] stageCFPrefixes)
		...
		if (create) {
			...
			for(String suffix: tableSuffixes) {
				if (!accumuloConnector.tableExists(tablePrefix+name+suffix)) {
					throw new RebarException("Table "+tablePrefix+name+suffix+
											 " not found; is the name \""+name+
											 "\" correct?");
				}
			}
		}
		this.stageTable = new AccumuloStageTable(accumuloConnector, 
												 tablePrefix+name+STAGES_TABLE_SUFFIX, 
												 this);
	...
	public long getSize() throws RebarException {
		...
		for (String tableName: tableNames)
			size += accumuloConnector.getSize(tableName);
		return size;
	}
	...
	public Stage makeStage(String stageName, String stageVersion, 
						   ...
						   String description, boolean deleteIfExists) 
		throws RebarException
	{
