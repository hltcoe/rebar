...
public class IndexedVertex extends IndexedProto<Concrete.Vertex> {
    ...
    public static IndexedVertex build(Concrete.UUID uuid, ProtoIndex index) throws RebarException {
        ...
        if (cached != null)
            ...
            return new IndexedVertex((Concrete.Vertex) index.lookup(uuid), index);
    }
    ...
    public static IndexedVertex build(Concrete.Vertex vertex, ProtoIndex index) throws RebarException {
        ...
        if (cached != null)
            ...
            return new IndexedVertex(vertex, index);
    }

    public IndexedVertex(Concrete.Vertex vertex, ProtoIndex index) throws RebarException {
        super(vertex, index);
    ...
    /*
     * public List<IndexedEdge> getOutgoingEdges() { Concrete.UUID src =
     * getUuid(); for (Concrete.UUID dst: getProto().getNeighborList()) {
     * boolean reversed = (); } }
     * 
     * private IndexedEdge getEdge(Concrete.UUID src, Concrete.UUID dst) {
     * Concrete.EdgeId edgeId = IdUtil.buildEdgeId(src, dst); if (graphReader !=
     * null) { return graphReader.readEdge(edgeId); } else { foo; } }
     */
