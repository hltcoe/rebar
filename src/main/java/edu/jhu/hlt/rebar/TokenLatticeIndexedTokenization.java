...
public class TokenLatticeIndexedTokenization extends IndexedTokenization {
  ...
  /* package-private */TokenLatticeIndexedTokenization(Concrete.Tokenization tokenization, ProtoIndex index) throws RebarException {
    ...
    for (Arc a : protoObj.getLattice().getArcList()) {
      ...
      if (a.hasToken())
        if (tokenArcs.put(a.getToken().getTokenIndex(), a) != null)
          throw new RebarException("Multiple tokens have the same id " + a.getToken().getTokenIndex());
    }
  }
   ...
   */
  private void initTopoSortedStates() throws RebarException {
