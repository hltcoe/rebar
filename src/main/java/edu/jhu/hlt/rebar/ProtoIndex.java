...
public final class ProtoIndex {
  ...
  public void registerCallback(Message message, ReplaceProtoCallback callback) throws RebarException {
  ...
  private void replaceAndUpdate(Message oldMsg, Message newMsg, FieldDescriptor field, Message fieldValue) throws RebarException {
   ...
   */
  private void replaceMessage(Message oldMsg, Message newMsg) throws RebarException {
  ...
  private void recordModification(Message target, FieldDescriptor field, Message fieldValue) throws RebarException {
    ...
    if (uuid != null) {
      ...
      throw new RebarException("Invalid merge target (not reachable via non-" + "repeated fields from a message with a UUID");
    }
  }
  ...
  private void recordModification(ModificationTarget target, FieldDescriptor field, Message fieldValue) throws RebarException {
  ...
  private RebarException reportBadFieldTarget(Message m, String methodName) throws RebarException {
    Concrete.UUID uuid = IdUtil.getUUIDOrNull(m);
    if (uuid == null)
      return new RebarException(protoDebugString(m) + " is not a valid target " + "for " + methodName + " because it does not " + "have a UUID.");
    else {
      /*
       ...
       * protoDebugString(p.getValue())); }
       */
      return new RebarException(protoDebugString(m) + " not found inside " + protoDebugString(root));
    }
  }

  private RebarException reportDuplicateField(Message target, FieldDescriptor field) throws RebarException {
    throw new RebarException(protoDebugString(target) + " already has a " + field.getName() + " value");
  }
}
