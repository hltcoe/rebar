...
/**
 * An interface to a "stage table", which records the set of stages that have been generated for a given corpus or knowledge graph.
 * 
 * The accumulo table that backs each AccumuloStageTable can contain the following cells:
 * 
 * <pre>
...
class AccumuloStageTable {
  ...
  public Stage getStage(String stageName) throws RebarException {
    ...
    if (splitPos >= 0) {
      return getStage(stageName.substring(0, splitPos), stageName.substring(splitPos + 1));
    } else {
  ...
  public Stage addStage(String stageName, String stageVersion, Collection<Stage> dependencies, String description, boolean deleteIfExists)
    ...
    if (hasStage(stageName, stageVersion)) {
      ...
      if (deleteIfExists)
        ...
        throw new RebarException("Stage " + oldStage + " already exists " + "(stageId=" + oldStage.getStageId() + ")");
    }
    // Check that no dependencies have been deleted.
    for (Stage stage : dependencies) {
      if (stagesById.get(stage.getStageId()) == null)
        throw new RebarException("Stage " + stage + " not found -- " + "was it deleted?");
    }
    ...
    Stage stage = new AccumuloStage(stageName, stageVersion, owner, stageId, dependencies, description, false);
  ...
  public boolean hasStage(String stageName, String stageVersion) throws RebarException {
    readStageTable();
    return (stagesByName.containsKey(stageName) && stagesByName.get(stageName).containsKey(stageVersion));
  }
  ...
  public void markStagePublic(Stage stage) throws RebarException {
    ...
    for (Stage dep : stage.getDependencies()) {
      if (!dep.isPublic())
        throw new RebarException("Stage " + stage + " can not be made public -- " + "dependency " + dep + " is not public.");
    }
    BatchWriter writer = accumuloConnector.createBatchWriter(tableName);
    Mutation m = new Mutation(new Text(ByteUtil.fromInt(stage.getStageId())));
  ...
  private void readStageTable(boolean force) throws RebarException {
    ...
    while (iterator.hasNext()) {
      ...
      if (key.compareColumnFamily(STAGE_NAME_CF) == 0) {
        ...
        throw new RebarException("While reading stages table: unexpected key " + key.toString());
      }
    }
    if (row != null) {
      int stageId = ByteUtil.toInt(row.getBytes());
  ...
  private void readStageRow(int stageId, String stageName, String stageVersion, List<Integer> stageDepIds, String description, boolean isPublic)
      throws RebarException {
    // Sanity checks
    if (stageId <= 0)
    ...
    if (stagesById.containsKey(stageId)) {
      ...
      if (!oldStage.getStageName().equals(stageName))
        throw new RebarException("Stage name for stage " + stageId + " changed: " + oldStage.getStageName() + "->" + stageName);
      if (!oldStage.getStageVersion().equals(stageVersion))
        throw new RebarException("Stage version for stage " + stageId + " changed: " + oldStage.getStageVersion() + "->" + stageVersion);
      if (!oldStage.getDependencies().equals(new TreeSet<Stage>(stageDeps)))
        throw new RebarException("Dependencies for stage " + stageId + " changed: " + oldStage.getDependencies() + "->" + stageDeps);
    ...
    if (stagesByName.containsKey(stageName) && stagesByName.get(stageName).containsKey(stageVersion))
      throw new RebarException("Duplicate stage name: " + stageName + ":" + stageVersion + "old stage id="
          + stagesByName.get(stageName).get(stageVersion).getStageId() + ", new stage id=" + stageId);
    // Create the stage object
    Stage stage = new AccumuloStage(stageName, stageVersion, owner, stageId, stageDeps, description, isPublic);
    // Register it
    if (!stagesByName.containsKey(stageName))
      stagesByName.put(stageName, new HashMap<String, Stage>());
    stagesByName.get(stageName).put(stageVersion, stage);
