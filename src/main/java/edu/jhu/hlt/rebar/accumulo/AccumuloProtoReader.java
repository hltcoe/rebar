 ...
 * merges in zero or more additional protobuf fields (the output of different processing stage).
 */
/* package-private */
abstract class AccumuloProtoReader<ProtoObj extends Message, ProtoBuilder extends Message.Builder, Identifier, Wrapper extends IndexedProto<ProtoObj>> {
  // ----------------------------------------------------------------------
  ...
  // ----------------------------------------------------------------------
  /**
   * The stage-ids of all stages that should be merged in. This includes stages that were explicitly specified as well as any dependencies of those stages.
   */
  protected final Map<Integer, Stage> stageIds;

  /**
   * The stages that we're reading from (does not currently include dependencies of those stages).
   */
  ...
  // ----------------------------------------------------------------------
  // Constructor
  AccumuloProtoReader(AccumuloConnector accumuloConnector, String tableName, Collection<Stage> inputStages) throws RebarException {
  ...
  private class ProtoIterator implements Iterator<Wrapper> {
    /**
     * The scanner used to read in cells. Might be a simple scanner or a batch scanner.
     */
    ...
    public boolean hasNext() {
      if (iterator.hasNext()) {
        ...
        if (!closed)
          close(); // auto-close at end of iteration.
        return false;
      }
    }
    ...
    private ScannerBase createScanner(Collection<Identifier> subset) throws RebarException {
      ...
      scanner.addScanIterator(new IteratorSetting(WHOLE_ROW_ITERATOR_PRIORITY, "wholeRows", WholeRowIterator.class));
      return scanner;
    }
  ...
  protected ProtoObj getRoot(Identifier rootId, Map<Key, Value> decodedRow) throws RebarException {
  ...
  private Wrapper parseRow(Key k, Value v, boolean recordStageOwnership) throws RebarException {
    ...
    for (; cell != null; cell = getNextStageCell(cellIter)) {
      ProtoIndex.ModificationTarget target = new ProtoIndex.ModificationTarget(cell.getKey().getColumnQualifier().getBytes());
  ...
  /**
   * Return the next cell that contains the output of a stage. These are identified as cells whose column family begins with STAGE_CF_PREFIX.
   */
  ...
  /**
   * Merge the given "stageOutput" into the builder "target", and update the list "stageOwnership" to record the stage ownership of each added field.
   * 
