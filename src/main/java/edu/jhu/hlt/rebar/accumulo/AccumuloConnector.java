...
class AccumuloConnector {
  ...
  public BatchWriter createBatchWriter(String tableName) throws RebarException {
    if (this.released)
      throw new RebarException("This AccumuloConnector has been released!");
    try {
      BatchWriter batchWriter = connector.createBatchWriter(tableName, MAX_WRITE_MEMORY, MAX_WRITE_LATENCY_MS, NUM_WRITE_THREADS);
  ...
  private BatchDeleter createBatchDeleter(String tableName) throws RebarException {
    if (this.released)
      throw new RebarException("This AccumuloConnector has been released!");
    try {
      BatchDeleter batchDeleter = connector.createBatchDeleter(tableName, auths, NUM_DELETER_QUERY_THREADS, MAX_WRITE_MEMORY, MAX_WRITE_LATENCY_MS,
          NUM_WRITE_THREADS);
  ...
  public boolean hasIterator(String tableName, String name) throws RebarException {
    try {
  ...
  public void attachIterator(String tableName, IteratorSetting setting) throws RebarException {
    try {
  ...
  private static void deleteSingleton() throws RebarException {
    ...
    hdfs = null;
    for (BatchScanner s : batchScanners)
      s.close();
    batchScanners = null;
    for (BatchDeleter s : batchDeleters)
      s.close();
    batchDeleters = null;
    try {
      for (BatchWriter s : batchWriters)
        s.close();
      batchWriters = null;
    } catch (MutationsRejectedException e) {
      throw new RebarException(e);
  ...
  private static void createSingleton() throws RebarException {
    ...
    if (RebarConfiguration.useAccumuloMock())
      ...
      zki = new ZooKeeperInstance(RebarConfiguration.getAccumuloInstanceName(), RebarConfiguration.getZookeeperServer());
    try {
      connector = zki.getConnector(RebarConfiguration.getAccumuloUser(), RebarConfiguration.getAccumuloPassword());
    } catch (AccumuloException e) {
      throw new RebarException(e);
    } catch (AccumuloSecurityException e) {
      throw new RebarException(e);
