 ...
 * 
 * Each corpus is backed by some external shared resource (such as an Accumulo table or a directory on a filesystem), and any changes made to the corpus are
 * persistant.
 * 
 * Corpus objects are created using the factory class "Corpus.Factory". In particular, use Corpus.Factory.makeCorpus() to create a new corpus, and
 * Corpus.Factory.getCorpus() to get a connection to an existing corpus.
 */
public interface Corpus extends StagedDataCollection {
  ...
  /**
   * Return a new identifier set containing the identifiers listed in the specified file. The file should contain one identifier per line.
   */
  public Collection<String> readComIdSet(File filename) throws RebarException;

  /**
   * Permanently associate a name with a given identifier set. This identifier set can then be looked up using the method 'lookupComIdSet()'.
   */
  public void registerComIdSet(String name, Collection<String> idSet) throws RebarException;

  /**
   * Look up and return a symbolic identifier set that has been defined using the 'registerComIdSet()' method.
   */
  public Collection<String> lookupComIdSet(String name) throws RebarException;

  /**
   * Get a list of the names of all symbolic communication identifier sets that have been defined for this corpus.
   */
  ...
  /**
   * Interface used to initialize a new corpus by defining the set of communications that it contains.
   * 
  ...
  /**
   * Return a Corpus.Initializer that can be used to add Communication objects to this corpus.
   */
  ...
  interface Reader {
     ...
     * 
     * @param subset
     *          A collection identifying which set of communications should be returned by the iterator.
     **/
  ...
  /**
   * Return a Corpus.Reader that reads communications from this corpus.
   * 
   * @param stages
   *          A list of stages whose output should be included in the returned Communications.
   */
  ...
  /**
   * Return a Corpus.Writer that can be used to add the output of a processing stage to this corpus.
   */
  public Writer makeWriter(Stage stage) throws RebarException;

  /**
   * Interface used by RPC to write "diffs", i.e., monotonic changes to communications. Each diff consists of a dictionary that maps from UUIDs of objects
