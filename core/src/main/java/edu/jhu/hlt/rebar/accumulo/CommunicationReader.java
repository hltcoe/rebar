...
public class CommunicationReader extends AbstractCommunicationReader {
  ...
  public Communication byUuid (UUID uuid) throws Exception {
    try (AutoCloseableIterator<Communication> it = this.fromMainTable(uuid.toString());) {
      if (it.hasNext())
        return it.next();
      else
  ...
  public AutoCloseableIterator<Communication> getCommunications(long unixTimeStart, long unixTimeEnd) throws RebarException {
  ...
  public AutoCloseableIterator<Communication> getCommunications(DateTime start, DateTime end) throws RebarException {
  ...
  public AutoCloseableIterator<Communication> getCommunications(DateTime past) throws RebarException {
    return this.getCommunications(past, new DateTime());
  }
  
  public AutoCloseableIterator<Communication> getCommunications(long unixTimePast) throws RebarException {
    return this.getCommunications(new DateTime(unixTimePast * 1000), new DateTime());
  }
  
  public AutoCloseableIterator<Communication> getCommunications(String t) throws RebarException {
   ...
   */
  @Override
  public AutoCloseableIterator<Communication> getAll() throws RebarException {
